...
void AudioSystem::Startup()
{
	...
	g_theEventSystem->SubscribeEventCallbackFunction("MuteAll", AudioSystem::MuteAll);
	g_theEventSystem->SubscribeEventCallbackFunction("UnMuteAll", AudioSystem::UnMuteAll);
}
...
//-----------------------------------------------------------------------------------------------
SoundID AudioSystem::CreateOrGetSound( const std::string& soundFilePath, bool is2DSound )
{
...
SoundPlaybackID AudioSystem::StartSound( SoundID soundID, bool isLooped, float volume, float balance, float speed, bool isPaused )
{
	...
	SoundPlaybackID PlayBackID = (SoundPlaybackID) channelAssignedToSound;
	m_registeredSoundPlaybackIDs.push_back(PlayBackID);
	return PlayBackID;
}
...
void AudioSystem::SetSoundPlaybackPaused(SoundPlaybackID soundPlaybackID, bool isPaused /*= false*/)
{
	if (soundPlaybackID == MISSING_SOUND_ID)
	{
		ERROR_RECOVERABLE("WARNING: attempt to set balance on missing sound playback ID!");
...
{
	m_isMuted = true;
	for (SoundPlaybackID soundID : m_registeredSoundPlaybackIDs)
	{
		SetSoundPlaybackSpeed(soundID, 0.0f);
	}
}
...
{
	//FMOD_RESULT result = m_fmodSystem->get3DNumListeners(&listenerIndex);
	//ValidateResult(result);

	FMOD_VECTOR listenerPositionFMOD;
	FMOD_VECTOR	listenerVelocityFMOD;
	FMOD_VECTOR listenerForwardMOD;
	FMOD_VECTOR	listenerUpMOD;

	listenerPositionFMOD.x = -listenerPosition.y;
	listenerPositionFMOD.y = listenerPosition.z;
	listenerPositionFMOD.z = -listenerPosition.x;

	listenerForwardMOD.x = -listenerForward.y;
	listenerForwardMOD.y = listenerForward.z;
	listenerForwardMOD.z = -listenerForward.x;

	listenerVelocityFMOD.x = 0.0f;
	listenerVelocityFMOD.y = 0.0f;
	listenerVelocityFMOD.z = 0.0f;

	listenerUpMOD.x = -listenerUp.y;
	listenerUpMOD.y = listenerUp.z;
	listenerUpMOD.z = -listenerUp.x;

	m_fmodSystem->set3DListenerAttributes(listenerIndex, &listenerPositionFMOD, &listenerVelocityFMOD, &listenerForwardMOD, &listenerUpMOD);
}
...
SoundPlaybackID AudioSystem::StartSoundAt(SoundID soundID, const Vec3& soundPosition, bool isLooped /*= false*/, float volume /*= 1.0f*/, float balance /*= 0.0f*/, float speed /*= 1.0f*/, bool isPaused /*= false*/)
{
	...
	if (channelAssignedToSound)
	{
		...
		if (m_isMuted)
		{
			channelAssignedToSound->setVolume(0.0f);
		}
		else
		{
			channelAssignedToSound->setVolume(volume);
		...
		FMOD_VECTOR soundPositionFMOD;
		FMOD_VECTOR soundVelocityFMOD;
		soundPositionFMOD.x = -soundPosition.y;
		soundPositionFMOD.y = soundPosition.z;
		soundPositionFMOD.z = -soundPosition.x;

		soundVelocityFMOD.x = 0.0f;
		soundVelocityFMOD.y = 0.0f;
		soundVelocityFMOD.z = 0.0f;
...
bool AudioSystem::MuteAll(EventArgs const& args)
{
	UNUSED(args);
	...
	g_theAudio->MuteAllSounds();
	return true;
}

bool AudioSystem::UnMuteAll(EventArgs const& args)
{
	UNUSED(args);
	if (g_theAudio == nullptr)
	{
...
AudioConfig const& AudioSystem::GetConfig() const
{
	return m_config;
}
...
void AudioSystem::UnmuteAllSounds()
{
	...
	for (SoundPlaybackID soundID : m_registeredSoundPlaybackIDs)
	{
		SetSoundPlaybackSpeed(soundID, 1.0f);
	}
