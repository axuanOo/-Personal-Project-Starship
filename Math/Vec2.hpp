...
public:
	...
	Vec2 const  GetRotatedRadians(float delatRadians ) const;
	Vec2 const	GetRotatedDegrees(float delatDegrees ) const;
	Vec2 const	GetClamped( float maxLength ) const;
	Vec2 const	GetNormalized() const;
	Vec2 const	GetReflected(Vec2 const& impactVectorNormalized) const;
	...
	void		Normalized();
	void		Reflect(Vec2 const& vectorToReflect);
	void		SetFromText(char const* text);
	float		NormalizedAndGetPreviousLength();
